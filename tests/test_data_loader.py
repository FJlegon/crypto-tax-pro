...
from src.data_loader import (
    ...
    _load_coinbase_csv,
    _parse_binance_date,
    _map_binance_transaction_type,
    _load_binance_csv,
    _parse_binance_market_pair,
    validate_file,
    load_ledgers,
)
...
# ─── Binance Transaction Type Mapping Tests ──────────────────────────────────

class TestBinanceTransactionTypeMapping:
    """Test Binance transaction type to LedgerEntry type/subtype mapping."""

    def test_buy_maps_to_trade_buy(self):
        """Buy transaction should map to type='trade', subtype='buy'."""
        entry_type, subtype = _map_binance_transaction_type("Buy")
        assert entry_type == "trade"
        assert subtype == "buy"

    def test_sell_maps_to_trade_sell(self):
        """Sell transaction should map to type='trade', subtype='sell'."""
        entry_type, subtype = _map_binance_transaction_type("Sell")
        assert entry_type == "trade"
        assert subtype == "sell"

    def test_deposit_maps_to_deposit(self):
        """Deposit transaction should map to type='deposit'."""
        entry_type, subtype = _map_binance_transaction_type("Deposit")
        assert entry_type == "deposit"
        assert subtype == ""

    def test_withdrawal_maps_to_withdrawal(self):
        """Withdrawal transaction should map to type='withdrawal'."""
        entry_type, subtype = _map_binance_transaction_type("Withdrawal")
        assert entry_type == "withdrawal"
        assert subtype == ""

    def test_reward_maps_to_earn_reward(self):
        """Reward transaction should map to type='earn', subtype='reward'."""
        entry_type, subtype = _map_binance_transaction_type("Reward")
        assert entry_type == "earn"
        assert subtype == "reward"

    def test_unknown_type_maps_to_unknown(self):
        """Unknown transaction type should map to 'unknown'."""
        entry_type, subtype = _map_binance_transaction_type("UnknownType")
        assert entry_type == "unknown"
        assert subtype == ""


# ─── Binance Date Parsing Tests ─────────────────────────────────────────────

class TestBinanceDateParsing:
    """Test Binance date parsing."""

    def test_parse_with_time(self):
        """Parse date with time: '2024-01-01 10:30:45'."""
        result = _parse_binance_date("2024-01-01 10:30:45")
        assert result is not None
    ...
    def test_parse_date_only(self):
        ...
        result = _parse_binance_date("2024-01-01")
        assert result is not None
        assert result.year == 2024
        assert result.month == 1
        assert result.day == 1
    ...
    def test_parse_empty_returns_none(self):
        """Empty string should return None."""
        result = _parse_binance_date("")
        assert result is None

    def test_parse_invalid_returns_none(self):
        """Invalid format should return None."""
        result = _parse_binance_date("not-a-date")
        assert result is None


# ─── Binance Market Pair Parsing Tests ───────────────────────────────────────

class TestBinanceMarketPairParsing:
    """Test Binance market pair parsing."""

    def test_parse_btc_usdt(self):
        """Parse 'BTC/USDT' to 'BTC'."""
        result = _parse_binance_market_pair("BTC/USDT")
        assert result == "BTC"

    def test_parse_eth_usdt(self):
        """Parse 'ETH/USDT' to 'ETH'."""
        result = _parse_binance_market_pair("ETH/USDT")
        assert result == "ETH"

    def test_parse_no_slash(self):
        """Handle market without slash."""
        result = _parse_binance_market_pair("BTC")
        assert result == "BTC"


# ─── Binance CSV Loading Tests ───────────────────────────────────────────────

class TestLoadBinanceCSV:
    """Test full Binance CSV loading."""

    def test_load_buy_transaction(self):
        """Load a Buy transaction from Binance CSV."""
        df = pd.DataFrame([
            {
        ...
        entries = _load_binance_csv(df, "binance_wallet")
        ...
        assert entry.asset == "BTC"
        assert entry.amount == Decimal("0.5")
        assert entry.amountusd == Decimal("22500")
        assert entry.fee == Decimal("22.5")

    def test_load_sell_transaction(self):
        """Load a Sell transaction from Binance CSV."""
        df = pd.DataFrame([
            {
                "Date": "2024-01-20",
                "Type": "Sell",
    ...
    def test_skip_invalid_date(self):
        """Rows with invalid dates should be skipped."""
        df = pd.DataFrame([
            {
                "Date": "invalid",
                "Type": "Buy",
...
# ─── Binance Validation Tests ───────────────────────────────────────────────

class TestBinanceValidation:
    """Test Binance file validation."""

    def test_validate_valid_binance_csv(self, tmp_path):
        """Valid Binance CSV should pass validation."""
        csv_file = tmp_path / "binance_test.csv"
        csv_file.write_text(
            "Date,Type,Market,Amount,Price,Total,Fee\n"
            "2024-01-15 10:30:45,Buy,BTC/USDT,0.5,45000,22500,22.5\n"
        )

        result = validate_file(str(csv_file), "binanceus")
        assert result.status in ("valid", "warning")

    def test_validate_missing_required_columns(self, tmp_path):
        """Missing required columns should fail validation."""
        csv_file = tmp_path / "binance_invalid.csv"
        csv_file.write_text(
            "Date,Type\n"
            "2024-01-15,Buy\n"
        )
...
# ─── Integration: load_ledgers with Binance ─────────────────────────────────

class TestLoadLedgersBinance:
    """Integration test: load_ledgers with Binance exchange."""

    def test_load_ledgers_binance(self, tmp_path):
        """Test load_ledgers with Binance exchange key."""
        csv_file = tmp_path / "binance.csv"
        csv_file.write_text(
            "Date,Type,Market,Amount,Price,Total,Fee\n"
            "2024-01-15 10:30:45,Buy,BTC/USDT,0.5,45000,22500,22.5\n"
            "2024-01-20,Sell,ETH/USDT,2.0,3000,6000,6.0\n"
        )

        entries = load_ledgers(str(csv_file), wallet_id="BinanceUS", exchange_key="binanceus")

        assert len(entries) == 2
        assert entries[0].wallet_id == "BinanceUS"
        assert entries[0].type == "trade"
        assert entries[1].wallet_id == "BinanceUS"
        assert entries[1].type == "trade"
